...
public class HumanServiceTools {
    ...
    @Tool(description = "创建人工服务单。仅当用户明确要求人工客服、投诉升级、情绪明显不满，或问题经客服/工具处理后仍难以解决时使用。写入操作：需先让用户核对原因，可关联订单号或售后单号。")
    public Map<String, Object> createHumanService(
            ...
            @ToolParam(description = "关联订单号(order_no)，没有则留空") String orderNo,
            @ToolParam(description = "关联售后单号(after_sale_no)，没有则留空") String afterSaleNo) {
        return humanServiceService.create(userId, reason, orderNo, afterSaleNo);
    }
}
